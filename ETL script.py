"""
Purpose: (i) ETL of EDGAR company data and stock price, and
         (ii) proccessing of financial indicators and predicted stock price.
"""
...
'''
# Include this code to drop table when need to change table structure (eg. change column name)
from sqlalchemy import text
with engine.connect() as conn:
    conn.execute(text(f"DROP TABLE IF EXISTS {database_table_name}"))
    conn.commit() 
'''
...
fallback_map = {
        ...
        'IncomeLossFromContinuingOperationsBeforeIncomeTaxesExtraordinaryItemsNoncontrollingInterest': ['IncomeLossFromContinuingOperationsBeforeIncomeTaxesExtraordinaryItemsNoncontrollingInterest', # Standard
...
def get_total_sga(data_source, name):
    ...
    if (dfi_ga is None and dfi_sm is None) and (ticker in ['BRK-A', 'BRK-B']):        
...
# eg. ORCL it does not have recurrent profit tag data, need to back calculate using non-operating income
    ...
    if not non_op_1.empty or not non_op_2.empty:
        if not non_op_1.empty:
            used_tag_list.append(used_tag_1)
        if not non_op_2.empty:
            used_tag_list.append(used_tag_2)
            
        non_op_1 = non_op_1['non_operating_income'].reindex(dfiRaw.index).fillna(0)
        non_op_2 = non_op_2['other_non_operating_income'].reindex(dfiRaw.index).fillna(0)
...
for index, row in companyInfo.iloc[:].iterrows():
    ...
    try:
        ...
        print(f"Processing company #{index}: {cik} - {ticker} ({company_name})")
        # =====================================================================
...
run_log_df = run_log_df.reset_index()
run_log_df = run_log_df.rename(columns={'index': 'source_index'})
run_log_df.to_sql('annual_financials_full_log', engine, if_exists='append', index=False)
